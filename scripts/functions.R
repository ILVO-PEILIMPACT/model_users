...
get_data_raw<-function(run_id, dir_run, file_sql){
  
  data_raw <-NULL
  for (s_run_id in run_id){
    
    #Read raw results
    file_raw <- str_c(dir_run, "/run_",formatC(x = s_run_id, format = "d", width = 9, flag = "0"),"/result_output.csv")
    crop_id = get_run_info_SQL(file_sql, s_run_id)$crop_id
    
    if (crop_id<=5) {
      db_temp <- read_csv_SWAP(file = file_raw, variable = c("PGRASSDM", "GRASSDM"))%>%
        rename(
          PDM = PGRASSDM,
          DM = GRASSDM)%>%
        mutate(
      ...
      db_temp <- read_csv_SWAP(file = file_raw, variable = c("CPWDM","CWDM"))%>%
        rename(
          PDM = CPWDM,
          DM = CWDM)%>%
        mutate(
      ...
      db_temp <- read_csv_SWAP(file = file_raw, variable = c("CPWSO","CWSO"))%>%
        rename(
          PDM = CPWSO,
          DM = CWSO)%>%
        mutate(
    ...
    data_raw <- rbind(data_raw, db_temp)
  }
...
get_data_procc<- function(run_id, dir_run, file_sql) {
  ...
  for (s_run_id in run_id){
    ...
    file_dir<-str_c(path,"/swap.swp")
    file_ind <-str_c(dir_run, "/run_",formatC(x = run_id_pot, format = "d", width = 9, flag = "0"),"/swap.swp")
